...
#SBATCH -J bowtie2_very_sens_loc_sort_dedup_MergeBam.JOBNAME
#SBATCH --mail-type=END,FAIL # notifications for job done & fail
#SBATCH --mail-user=philippa.borrill\@jic.ac.uk # send-to address
SLURM

 my $tmp_file = "$output_dir/tmp/bowtie2_very_sensitive_local_5_4_2016.$dir";
...
#source and run bowtie2 with paired ends, very-sensitive-local parameters 8 threads
  print SLURM "source bowtie2-2.2.4\n";
 #   print SLURM "bowtie2 --very-sensitive-local -p 8 -x $ref -1 $pair_1_R1 -2 $pair_1_R2 -S $output_dir/$output"."_n$mismatches.sam\n";
