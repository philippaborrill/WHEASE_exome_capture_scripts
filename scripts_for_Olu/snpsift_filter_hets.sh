...
# 3 steps separately
# 1st only keep variant sites
srun cat samtools_sets1-8_merged_filtered_0.75_indiv_cov.recode.vcf | java -jar /usr/users/metbio/borrillp/bin/snpEff/SnpSift.jar filter "( isVariant( GEN[*] ) )" > samtools_sets1-8_merged_filtered_0.75_indiv_variant_positions_only.vcf
...
srun cat samtools_sets1-8_merged_filtered_0.75_indiv_variant_positions_only.vcf | java -jar /usr/users/metbio/borrillp/bin/snpEff/SnpSift.jar filter "(countHet() < 2)" > samtools_sets1-8_merged_filtered_0.75_indiv_variant_positions_only_under2_hets.vcf

# only keep sites with 2 or fewer alleles
source vcftools-0.1.12
vcftools --vcf samtools_sets1-8_merged_filtered_0.75_indiv_variant_positions_only_under2_hets.vcf --out samtools_sets1-8_merged_filtered_0.75_indiv_variant_positions_only_under2_hets_max_2_alleles --max-alleles 2 --recode --recode-INFO-all
