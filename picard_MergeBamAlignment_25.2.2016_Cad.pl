...
## convert fastq to sam
  ...
  print BSUB "picard CleanSam INPUT=$output_dir/no_bam_header/$output"."_n5.sorted.bam OUTPUT=$output_dir/no_bam_header/$output"."_n5.sorted.cleaned.bam\n";
  print BSUB "picard MergeBamAlignment ALIGNED=$output_dir/no_bam_header/$output"."_n5.sorted.cleaned.bam UNMAPPED=$output_dir/$output.fastqtosam.bam OUTPUT=$output_dir/$output"."_MergeBamAlignment.bam REFERENCE_SEQUENCE=$ref\n\n";
