...
my $list_of_chrom_sizes = 'names_sizes_Triticum_aestivum.IWGSC1.0+popseq.30.dna_sm.genome_replaced_ambig_bases.txt';


#open the input file and go through the lines one by one so go to each directories where the fastq.gz should be located
chdir("$path") or die "couldn't move to output directory";

open (INPUT_FILE, "$list_of_chrom_sizes") || die "couldn't open the input file $list_of_chrom_sizes!";
