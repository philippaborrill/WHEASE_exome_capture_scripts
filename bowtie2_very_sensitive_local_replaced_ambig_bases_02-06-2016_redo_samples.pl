...
my $exome_capture_paired_list = 'input_for_bwa-aln_redo_samples.txt';
...
#source and run bowtie2 with paired ends, very-sensitive-local parameters 2 threads
  ...
  print SLURM "srun bowtie2 --very-sensitive-local -p 2 -x $ref -1 $pair_1_R1 -2 $pair_1_R2 -S $output_dir/$output"."_n$mismatches.sam\n";
...
# convert sam to bam and sort by coordinate, make flagstat
  ...
  print SLURM "srun samtools sort $output_dir/$output"."_n$mismatches.bam $output_dir/$output"."_n$mismatches.sorted\n";
...
# merge mapped bam with unmapped bam to get "clean" file, flagstat
  print SLURM "srun picard MergeBamAlignment ALIGNED=$output_dir/$output"."_n$mismatches.sorted.markdup_rm.bam UNMAPPED=$path/exome_capture_analysis/fastqtosam/$output.fastqtosam.bam OUTPUT=$output_dir/$output"."_MergeBamAlignment.bam VALIDATION_STRINGENCY=LENIENT REFERENCE_SEQUENCE=$ref2\n";
