...
#SBATCH -o $output_dir/slurm_output/slurm.picard_CleanSam_sort_dedup.JOBNAME.%N.%j.out # STDOUT
#SBATCH -e $output_dir/slurm_output/slurm.picard_CleanSam_sort_dedup.JOBNAME.%N.%j.err # STDERR
#SBATCH -J picard_CleanSam_sort_dedup_JOBNAME
#SBATCH --mail-type=END,FAIL # notifications for job done & fail
#SBATCH --mail-user=philippa.borrill\@jic.ac.uk # send-to address
SLURM

 my $tmp_file = "$output_dir/tmp/picard_CleanSam_sort_dedup_slurm_13-3-2016.$output";
...
## merge ubam and bam to get read header info
  ...
  print SLURM "samtools flagstat $output_dir/with_bam_header/$output"."_MergeBamAlignment.bam > $output_dir/with_bam_header/$output"."_MergeBamAlignment.flagstat.txt\n";

# clean sam to make unmapped reads have MAPQ0
  print SLURM "picard CleanSam INPUT=$output_dir/with_bam_header/$output"."_MergeBamAlignment.bam OUTPUT=$output_dir/with_bam_header/$output".".MergeBam.cleaned.bam\n";  
...
# remove duplicates
  ...
  print SLURM "samtools flagstat $output".".MergeBam.cleaned.markdup_rm.bam > $output".".MergeBam.cleaned.markdup_rm.flagstat.txt\n";
