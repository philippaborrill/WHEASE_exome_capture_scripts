...
srun cat freebayes_all_samples_together.vcf | java -jar /usr/users/metbio/borrillp/bin/snpEff/SnpSift.jar filter "(countHet() < 2)" > freebayes_all_samples_together_only_under2_hets.vcf

# only keep sites with 2 or fewer alleles
source vcftools-0.1.12
vcftools --vcf freebayes_all_samples_together_only_under2_hets.vcf --out freebayes_all_samples_together_only_under2_hets_max_2_alleles --max-alleles 2 --recode --recode-INFO-all
