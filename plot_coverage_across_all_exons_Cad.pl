...
print SLURM "srun /usr/users/metbio/borrillp/bin/samtools view -q 10 -u $output_dir/with_bam_header/$sample".".MergeBam.cleaned.markdup_rm.bam | bedtools coverage -hist -abam stdin -b $path/IWGSC_exons_only.gtf | grep ^all > $output_dir/with_bam_header/$sample"."bam.hist.all_q10.txt \n";
