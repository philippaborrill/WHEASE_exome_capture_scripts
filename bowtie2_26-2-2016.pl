...
LSF
 ...
 my $tmp_file = "$output_dir/tmp/bowtie2_lsf_26-2-2016.$dir";
...
# merge mapped bam with unmapped bam to get "clean" file, flagstat
  print BSUB "picard MergeBamAlignment ALIGNED=$output_dir/$output"."_n$mismatches.sorted.markdup_rm.bam UNMAPPED=$path/exome_capture_analysis/fastqtosam/$output.fastqtosam.bam OUTPUT=$output_dir/$output"."_MergeBamAlignment.bam REFERENCE_SEQUENCE=$ref2\n";
