...
print SLURM "srun /usr/users/metbio/borrillp/bin/samtools mpileup -D -A -Q 20 -q 20 -u -g -f $ref $output_dir/with_bam_header/$sample".".MergeBam.cleaned.markdup_rm.bam | /usr/users/metbio/borrillp/bin/bcftools view -bvcg - > $output_dir/with_bam_header/$sample".".MergeBam.cleaned.markdup_rm_var.raw.bcf \n";

# options for vcfutils.pl varFilter
# -D 1000 max read depth
# -a min alt base reads

print SLURM "/usr/users/metbio/borrillp/bin/bcftools view $output_dir/with_bam_header/$sample".".MergeBam.cleaned.markdup_rm_var.raw.bcf | /usr/users/metbio/borrillp/bin/vcfutils.pl varFilter -a 3 -D 1000 > $output_dir/with_bam_header/$sample".".MergeBam.cleaned.markdup_rm_var.flt.vcf \n";
