...
SLURM
 ...
 my $tmp_file = "$output_dir/tmp/gatk_subtract_31_3_2016.$output";

  open (SLURM, ">$tmp_file") or die "Couldn't open temp file\n";
  $SLURM_header =~ s/JOBNAME/$output/g;
  print SLURM "$SLURM_header\n\n";
  print SLURM "\ncd $output_dir/with_bam_header\n";

# source programmes
print SLURM "source GATK-3.5.0 \n";

#combine the data keeping only variants found in 2 or more Cadenza samples
print SLURM "srun gatk -T CombineVariants -R $ref -V:VCF variants_found_in_2_or_more_Cadenza_het_in_at_least_1_mutant.vcf -V:$sample $sample".".MergeBam.cleaned.markdup_rm.merged_markdup_rm_2nd_time_var.flt.vcf -o hetCad_with_$sample"."_merged.vcf \n";

print SLURM "srun gatk -T SelectVariants -R $ref -V:VCF hetCad_with_$sample"."_merged.vcf -select 'set == \"$sample\";' -o variants_in_$sample"."_not_in_Cad.vcf \n";
